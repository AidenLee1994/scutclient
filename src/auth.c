...
/* auth_8021x_Handler 解析 EAP 报文时访问到的最大固定偏移，
 * 收包长度不足则丢弃，避免读取越界/残留数据 */
#define AUTH_8021X_MIN_LEN 23
...
/* 静态常量*/
static const uint8_t BroadcastAddr[6] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }; // 广播MAC地址
static const uint8_t MultcastAddr[6] = { 0x01, 0x80, 0xc2, 0x00, 0x00, 0x03 }; // 多播MAC地址
static const uint8_t UnicastAddr[6] = { 0x01, 0xd0, 0xf8, 0x00, 0x00, 0x03 }; // 单播MAC地址
...
int auth_8021x_Receiver(uint8_t *recv_data) {
	...
	// 过滤掉非0x888e、目标非本机、或长度不足以解析EAP头的报文
	if (recv_len >= AUTH_8021X_MIN_LEN
...
int Authentication(int client) {
	...
	while (resev) {
		...
		tmp_timeout = timeout;
		switch (select(MAX(auth_8021x_sock, auth_udp_sock) + 1, &fdR, NULL, NULL,
				&tmp_timeout)) {
		case -1:
...
int auth_8021x_Handler(uint8_t recv_data[]) {
	...
	if ((EAP_Code) recv_data[18] == REQUEST) {
		...
		case NOTIFICATION:
			...
			// pkg_len来自网络报文，需校验以防越界写（终止符位置为 pkg_len+18）
			if (pkg_len < 5 || (23 + pkg_len - 5) >= ETH_FRAME_LEN) {
				LogWrite(DOT1X, ERROR, "Server: Invalid notification length (%u).", pkg_len);
				return -1;
			}
			// 在信息的最后补0，方便打印
		...
		sleep(AUTH_8021X_RECV_DELAY);
		if (OnlineHookCmd) {
			if (system(OnlineHookCmd) != 0)
				LogWrite(DOT1X, ERROR, "Online hook command failed.");
		}
