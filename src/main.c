...
int main(int argc, char *argv[]) {
	...
	LogWrite(ALL, INF, "由 Dr. Lee 提供技术支持");
	LogWrite(ALL, INF, "华南理工大学计算机科学与工程学院");
	LogWrite(ALL, INF, "#######################################");
	int client = 1;
	int ch, tmpdbg;
	...
	while ((ch = getopt_long(argc, argv, "u:p:i:n:H:s:c:T:h:E:Q:D::o",
		...
		case 'Q':
			...
			break;
		case 'i':
			strncpy(DeviceName, optarg, IFNAMSIZ - 1);
	...
	/* 校验用户名/密码长度：
	 * 用户名与主机名共用 MISC_INFO 报文中的 32 字节字段，
	 * 用户名 >= 32 会导致 (32 - strlen(UserName)) 下溢；
	 * 密码在 FillMD5Area 中拼入 128 字节缓冲区(1+密码+16)，需 < 111。*/
	if (client != LOGOFF) {
		if (strlen(UserName) >= 32) {
			LogWrite(INIT, ERROR, "Username too long (must be < 32 characters)!");
			exit(-1);
		}
		if (strlen(Password) >= 100) {
			LogWrite(INIT, ERROR, "Password too long (must be < 100 characters)!");
			exit(-1);
		}
	}
	...
	while(1) {
		...
		if(ret == 1) {
			...
			if(((int)a_hour * 60 + a_minute) > ((int)(cltime -> tm_hour) * 60 + cltime -> tm_min)) {
				...
				if (OfflineHookCmd) {
					if (system(OfflineHookCmd) != 0)
						LogWrite(ALL, ERROR, "Offline hook command failed.");
				}
